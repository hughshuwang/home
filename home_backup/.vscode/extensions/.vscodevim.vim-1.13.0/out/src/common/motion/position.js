...
class Position extends vscode.Position {
    ...
    static *IterateDocument(start, forward = true) {
        if (forward) {
            for (let lineIndex = start.line; lineIndex < textEditor_1.TextEditor.getLineCount(); lineIndex++) {
     ...
     */
    getLeftIfEOL() {
        if (this.character === textEditor_1.TextEditor.getLineLength(this.line)) {
            return this.getLeft();
        }
        else {
            return this;
    ...
    getDocumentEnd(textEditor) {
        const lineCount = textEditor_1.TextEditor.getLineCount(textEditor);
        const line = lineCount > 0 ? lineCount - 1 : 0;
        const char = textEditor_1.TextEditor.getLineLength(line);
