...
class ModeHandler {
    ...
    static Create(textEditor = vscode.window.activeTextEditor) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            yield modeHandler.setCurrentMode(configuration_1.configuration.startInInsertMode ? mode_1.Mode.Insert : mode_1.Mode.Normal);
    ...
    handleSelectionChange(e) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (this.vimState.currentMode === mode_1.Mode.SearchInProgressMode ||
                this.vimState.currentMode === mode_1.Mode.CommandlineInProgress) {
                return;
            }
            let toDraw = false;
            if (selection) {
                ...
                if (newPosition.character >= newPosition.getLineEnd().character && selection.isEmpty) {
                    if (this.vimState.currentMode !== mode_1.Mode.Insert) {
                        ...
                        yield this.setCurrentMode(mode_1.Mode.Normal);
                        toDraw = true;
                    }
                }
                ...
                if (selection.anchor.line === selection.active.line &&
                    ...
                    if (configuration_1.configuration.mouseSelectionGoesIntoVisualMode &&
                        ...
                        yield this.setCurrentMode(mode_1.Mode.Visual);
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                ...
                if (e instanceof error_1.VimError) {
                    statusBar_1.StatusBar.setText(this.vimState, e.toString(), true);
                }
                else {
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            */
            if (vimState.currentMode === mode_1.Mode.Visual) {
                vimState.cursors = vimState.cursors.map(x => x.start.isBefore(x.stop) ? x.withNewStop(x.stop.getLeftThroughLineBreaks(true)) : x);
            }
            ...
            if (vimState.currentMode !== this.currentMode) {
                ...
                if (vimState.currentMode === mode_1.Mode.Normal &&
                    prevState !== mode_1.Mode.SearchInProgressMode &&
                    prevState !== mode_1.Mode.CommandlineInProgress &&
                    prevState !== mode_1.Mode.EasyMotionInputMode &&
                    prevState !== mode_1.Mode.EasyMotionMode) {
                    ranRepeatableAction = true;
                }
            }
            // Set context for overriding cmd-V, this is only done in search entry and
            // commandline modes
            if (this.IsModeWhereCmdVIsOverridden(vimState.currentMode) &&
                !this.IsModeWhereCmdVIsOverridden(prevState)) {
                yield vscode_context_1.VsCodeContext.Set('vim.overrideCmdV', true);
            }
            else if (this.IsModeWhereCmdVIsOverridden(prevState) &&
            ...
            if (vimState.currentMode === mode_1.Mode.Visual) {
                vimState.cursors = vimState.cursors.map(x => x.start.isBefore(x.stop)
            ...
            if ((movement && !movement.preservesDesiredColumn()) ||
                (!movement && vimState.currentMode !== mode_1.Mode.VisualBlock && !action.preservesDesiredColumn())) {
            ...
            if (ranAction) {
                ...
                if (vimState.returnToInsertAfterCommand) {
                    if (vimState.actionCount > 0) {
                        ...
                        yield this.setCurrentMode(mode_1.Mode.Insert);
                    }
                    else {
            ...
            if (!this.vimState.focusChanged) {
                ...
                else if (!configuration_1.configuration.experimentalOptimizations || mightChangeDocument) {
                    const addedChange = vimState.historyTracker.addChange(this.vimState.cursorsInitialState.map(c => c.stop));
    ...
    executeOperator(vimState) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            for (let { start, stop } of vimState.cursors) {
                ...
                if (!mode_1.isVisualMode(startingModeName) && cachedRegister !== register_1.RegisterMode.LineWise) {
            ...
            for (const command of otherTransformations) {
                switch (command.type) {
                    ...
                    case 'showCommandHistory':
                        let cmd = yield commandLine_1.commandLine.showHistory(vimState.currentCommandlineText);
                        if (cmd && cmd.length !== 0) {
                    ...
                    case 'showSearchHistory':
                        ...
                        if (searchState) {
                            ...
                            if (!nextMatch) {
                                throw error_1.VimError.fromCode(command.direction > 0 ? error_1.ErrorCode.SearchHitBottom : error_1.ErrorCode.SearchHitTop);
                            }
                    ...
                    case 'dot':
                        ...
                        yield this.rerunRecordedState(vimState, globalState_1.globalState.previousFullAction.clone());
                        break;
                    case 'macro':
    ...
    updateView(vimState, args = {
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (args.drawSelection) {
                ...
                if (vimState.currentMode === mode_1.Mode.CommandlineInProgress) {
                    selectionMode = commandLine_1.commandLine.previousMode;
                }
                if (!vimState.isMultiCursor) {
                    let start = vimState.cursorStartPosition;
                    let stop = vimState.cursorStopPosition;
                    if (selectionMode === mode_1.Mode.Visual) {
                        /**
                    ...
                    else if (selectionMode === mode_1.Mode.VisualLine) {
                        selections = [
                    ...
                    else if (selectionMode === mode_1.Mode.VisualBlock) {
                        selections = [];
                        for (const { start: lineStart, end } of position_1.Position.IterateLinesInBlock(vimState)) {
                    ...
                    switch (selectionMode) {
                        ...
                        case mode_1.Mode.VisualLine: {
                            selections = vimState.cursors.map((c) => {
                        ...
                        case mode_1.Mode.VisualBlock: {
                            for (const c of vimState.cursors) {
                                for (const { start: lineStart, end } of position_1.Position.IterateLinesInBlock(vimState, c)) {
                                    selections.push(new vscode.Selection(lineStart, end));
                                }
                            }
                            break;
            ...
            if (vimState.editor.visibleRanges.length > 0 &&
                ...
                const centerViewportAroundCursor = visibleRange.start.line - vimState.cursorStopPosition.line >= 15 ||
            ...
            // cursor style
            const modeName = mode_1.Mode[this.currentMode];
            let cursorStyle = configuration_1.configuration.getCursorStyleForMode(modeName);
            if (!cursorStyle) {
            ...
            */
            ...
            yield vscode_context_1.VsCodeContext.Set('vim.mode', mode_1.Mode[this.vimState.currentMode]);
    ...
    IsModeWhereCmdVIsOverridden(mode) {
        return [mode_1.Mode.SearchInProgressMode, mode_1.Mode.CommandlineInProgress].includes(mode);
    }
}
