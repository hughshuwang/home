...
class RecordedState {
     ...
     */
    get operator() {
        let list = this.actionsRun.filter(a => a instanceof operator_1.BaseOperator).reverse();
        return list[0];
    }
    get operators() {
        return this.actionsRun.filter(a => a instanceof operator_1.BaseOperator).reverse();
    }
    /**
     * The command (e.g. i, ., R, /) the user wants to run, if there is one.
     */
    get command() {
        const list = this.actionsRun.filter(a => a instanceof actions_1.BaseCommand).reverse();
    ...
    operatorReadyToExecute(mode) {
        ...
        return (this.operator &&
            ...
            (this.hasRunAMovement ||
                mode === mode_1.Mode.Visual ||
                mode === mode_1.Mode.VisualLine ||
